...
const StaffAdd = ({ closeAdd }) => {
    ...
    const createStaff = (e) => {
        e.preventDefault();
        const staffs = { name, email, password };

        Staff.addStaff(staffs).then((response) => {
        ...
        <div className='fixed inset-0 bg-black opacity-95 z-10 backdrop-blur-xl flex justify-center items-center'>
            <div className=" bg-slate-300 w-72 blur-none h-auto opacity-100 text-black p-2 rounded-2xl">
                <div className='flex flex-col-reverse gap-1 mb-3'>
                    <h3 className='text-center absolute top-5 right-0 left-0'>Add New Staff</h3>
                ...
                <div className='flex justify-center gap-3'>
                    ...
                    <button className='w-44 mx-auto hover:bg-orange-600' onClick={createStaff} type='submit'>Register</button>
                </div>
            </div>
        </div>
    )
