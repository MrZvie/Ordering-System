...
const Update = () => {
    ...
    Staff.updateStaff(id).then(response => {
        ...
        <div className='fixed inset-0 bg-black opacity-80 backdrop-blur-xl flex justify-center items-center'>
            <div className=" bg-slate-300 w-72 h-auto text-black p-2 rounded-2xl">
                ...
                <div className='flex justify-center gap-3'>
                    <Link to={'http://localhost:5173/admin/dashboard/staffinfo/'}><button className='hover:bg-primary border-primary hover:text-white w-24'>
                        Cancel
                    </button></Link>
                    <button onClick={() => updateUser(staff.id)} className='hover:bg-primary border-primary hover:text-white w-24'>Update</button>
                </div>
            </div>
        </div>
    )
